...
export async function scrapeDetail(slug: string): Promise<VideoDetail> {
  ...
  // Scope ke .entry-content agar tidak match script dari widget/footer.
  let videoSrc: string | null = null;

  // 1. JW Player setup script — scope ke .entry-content
  $(".entry-content script:not([src])").each((_, el) => {
    if (videoSrc) return;
    const scriptContent = $(el).html() ?? "";
    if (!scriptContent.includes("jwplayer")) return;
    const match =
      scriptContent.match(/["']file["']\s*:\s*["'](https?:\/\/[^"']+\.mp4[^"']*)['"]/i) ??
      scriptContent.match(/file\s*:\s*["'](https?:\/\/[^"']+\.mp4[^"']*)['"]/i);
    if (match?.[1]) videoSrc = match[1];
  });

  // 2. Fluid Player — <video id="my-video"><source src="..."> (mobile theme)
  if (!videoSrc) {
    const videoEl = $("video#my-video").first();
    videoSrc =
      videoEl.find("source[src]").first().attr("src") ??
      videoEl.attr("src") ??
      $(".entry-content video source[src]").first().attr("src") ??
      null;
  }

  const iframe = $("iframe").first();
  const iframeUrl: string | null = iframe.attr("src") ?? null;
  const videoUrl: string | null = videoSrc ?? iframeUrl;
